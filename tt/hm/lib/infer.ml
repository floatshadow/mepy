    ...
    *)
    ...
    (* 推导一个名字的类型只需要我们在环境 ctx 中进行查找即可。没有要生成的约束，也没引入新的未知类型
       ...
       --------------------
        ctx |- n: lookup ctx n -| {}
    *)
    | Name name -> (lookup ctx name, empty)
    ...
    *)
    | Lam (x, e) ->
      let t1 = next_fresh_tvar fresh in
        let t2, c = collect fresh (extend ctx x @@ t1) e in
          (TLam (t1, t2), c)
    (* 应用的规则如下。
         可能有人会产生疑惑，
  ...
  let rec unify (cs : constr list) : subst list =
    ...
    *)
    | (Constr (_t1, _t2) as _c) :: _rest ->
      ...
      | TVar x, _ when not @@ contains _t2 x ->
        let _s = x => _t2 in
          _s :: (subst_all_in_constr _s _rest |> unify)
      | _, TVar y when not @@ contains _t1 y ->
        let _s = y => _t1 in
          _s :: (subst_all_in_constr _s _rest |> unify)
      | TLam (_i1, _o1), TLam(_i2, _o2) ->
        unify ( [_i1 =~ _i2; _o1 =~ _o2] @ _rest)
      | _ ->
...
module Infer = struct
  ...
  *)
  let instantiate fresh (Ts (xs, t)) : ty =
    List.fold_left xs
    ~init: t 
    ~f: (fun _t _x -> 
      let _tvar = next_fresh_tvar fresh in
        subst (_x => _tvar) _t)
  ...
  let rec collect fresh ctx expr : ty * constr list =
    ...
    | Name x -> 
      let instantiated_ty = instantiate fresh @@ lookup ctx x in
      (instantiated_ty, empty)
    (* 照旧 *)
    | If (e1, e2, e3) ->
        let t1, c1 = collect fresh ctx e1 in
        let t2, c2 = collect fresh ctx e2 in
    ...
    *)
    | Lam (x, e) ->
      let t1 = next_fresh_tvar fresh in
        let t2, c = collect fresh (extend ctx x @@ Ts ([], t1)) e in
          (TLam (t1, t2), c)
    (* 照旧 *)
    | App (e1, e2) ->
      let t3 = next_fresh_tvar fresh in
    ...
    *)
    | Let (x, e1, e2) ->
      let t1, c1 = collect fresh ctx e1 in
        let ktx = generalize ctx x t1 c1 in
          let t2, c2 = collect fresh ktx e2 in
            (t2, c2)
  ...
  and generalize ctx (x : string) (t : ty) (c : constr list) : tyscheme ctx =
    let sts = unify c in
      ...
      let ktx = subst_all_in_ctx sts ctx in
        ...
        let ref_raw_tvars = ref [] in
        let rec find_vars ty =
          match ty with
          | TInt -> TInt
          | TBool -> TBool
          | TLam (t1, t2) -> TLam(find_vars t1, find_vars t2)
          | TVar x ->
            ref_raw_tvars := x :: !ref_raw_tvars;
            TVar x
          in
          let _u = find_vars u in

          let rec remove_dup ts vars =
            match vars with
            | [] -> ts
            | x :: xs -> if Option.is_none @@ List.Assoc.find ktx x ~equal: String.equal && 
                            not @@ List.mem ts x ~equal: String.equal 
                         then remove_dup (x :: ts) xs
                         else remove_dup ts xs
            in
            let tvars = remove_dup [] !(ref_raw_tvars) in
              extend ktx x @@ Ts (tvars, u)

  (* Recall that generalization `generial Γ t` of the type `t` with respect to the type environment  `Γ` is quantifying free type variables of `t` that do not occur as free in `Γ`.
    generalize Γ x : t = Γ x : (∀ α1, ..., αn. t) where {α1 ... αn} = FV(t) - FV(Γ).

    ML adds a condition for generalization, so-called value restriction: the let-bound expression `e1`, by the look of it, must have no visible side-effects -- technically, `e1` must pass the syntactic test of being _nonexpansive_.
    
    In the above naive implementation, we simply scan all the type variables in the `Γ`, which can be very slow when `Γ` is large.
  *)
