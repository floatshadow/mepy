...
(*
  System D is the normal modal logic K plus the seriality axiom:

      □ φ ⟶ ◇ φ

  This file is meant to be read beside ModalK.v.

  The comparison with System K is the main point:

  - Syntax: exactly the same formulas as K.
  - Rules: exactly the same rules as K, plus one axiom schema ax_D.
  - Semantics: K quantifies over arbitrary Kripke frames; D quantifies only
    over serial Kripke frames, where every world has a successor.
  - Soundness: every K soundness case is reused unchanged. The only new case
    is ax_D_sound, and the only new semantic fact it needs is seriality.
...
(*
  ...
  This keeps the language identical to System K. System D is stronger because
  of its additional axiom and frame condition, not because it has more syntax.
*)
...
(*
  Hilbert-style derivability for D.
  ...
  Intuitively, if φ is necessary, then φ is possible. This is not valid on all
  K frames: at a world with no accessible successors, □ φ is vacuously true
  while ◇ φ is false. System D rules out exactly those dead-end worlds by
  requiring seriality in the semantics.
...
(*
  Kripke models for D use serial frames.

  World  : the type of possible worlds
  access : accessibility relation between worlds
  serial : every world has at least one accessible successor
  val    : valuation of atoms at worlds

  This is the semantic difference from System K. A K model has World, access,
  and val, with no condition on access. A D model adds:

      serial : forall w, exists v, access w v

  So every D model is K-like, but not every K model is a D model. The excluded
  K models are precisely those with dead-end worlds. Those are exactly the
  frames where the D axiom can fail.
*)
  Record model : Type := {
    World : Type;
    access : World -> World -> Prop;
    serial : forall w : World, exists v : World, access w v;
    val : World -> Atom -> Prop;
  }.
...
(*
  ...
  Semantic entailment Γ ⊨ φ quantifies over all serial models and worlds.

  Compare this with System K: the definition of entailment has the same shape,
  but the type of M is now the D model type above. Therefore every semantic
  statement in this section is automatically restricted to serial frames.
*)
...
(*
  Each axiom schema is valid in every serial Kripke model.
  ...
  The D axiom is the only axiom whose soundness proof actually uses the extra
  serial field of the model.
*)
  ...
  Lemma ax_D_sound Γ φ :
    Γ ⊨ □ φ ⟶ ◇ φ.
  Proof.
    intros M w _. simpl.
    intros Hboxφ Hboxnotφ.
(*
    This is the whole K-versus-D step.

    After unfolding ◇ φ as ¬ □ ¬ φ, we must refute the assumption Hboxnotφ:

        Hboxnotφ : M, w ⊩ □ ¬ φ

    From Hboxφ alone we cannot produce a contradiction unless w has an
    accessible successor. K allows w to have no successors, so the proof would
    get stuck there. D gives seriality, so we pick an accessible v and obtain
    both φ and ¬ φ at v.
*)
    destruct (serial M w) as [v Hwv].
    exact (Hboxnotφ v Hwv (Hboxφ v Hwv)).
  Qed.
...
(*
  ...
  branch for ax_D. That branch is discharged by ax_D_sound, whose proof is the
  only place where seriality is used.
*)
